...
def get_model(model_name, root_path=default_root_path):
    ...
    Args:
        ...
        root_path (_type_, optional): _description_. Defaults to default_root_path.

    Raises:
        NotImplementedError: If the name of the model is not recognized
    ...
    if model_name == 'wiki80_cnn_softmax':
    ...
    elif model_name == 'custom_trained_model':
        ...
        if len(files)>0:
            ...
            for k, v in files.items():
                print(f"{k+1}. {v}")
            selected_model = ""
            while selected_model not in files.keys():
                try:
                    selected_model = int(input("Enter the no. of the model you want to use: "))-1
                except:
                    print("Please a enter valid input (INTEGERS only)")
                    print("Try Again!")
                else:
                    print("Selected mode:", files[selected_model])
                    selected_model_name = files[selected_model]
        if selected_model_name is not None:
            ckpt = os.path.join(os.getcwd(), f"ckpt/{selected_model_name}")
            download('bert_base_uncased', root_path=root_path)
            rel2id = json.load(open('benchmark/wiki80/wiki80_rel2id.json'))
            pretrain_path=os.path.join(root_path, 'pretrain/bert-base-uncased')
            sentence_encoder = encoder.BERTEntityEncoder(max_length=80, pretrain_path=pretrain_path)
            m = model.SoftmaxNN(sentence_encoder, len(rel2id), rel2id)
            m.load_state_dict(torch.load(ckpt, map_location='cpu')['state_dict'])
            return m
        else:
            print("No custom trained models are available")
            
    elif model_name=='custom_model':
        ckpt = os.path.join(os.getcwd(), "ckpt/wiki80_bert-base-uncased_entity.pth.tar")
        download('bert_base_uncased', root_path=root_path)
        rel2id = json.load(open('benchmark/wiki80/wiki80_rel2id.json'))
        pretrain_path=os.path.join(root_path, 'pretrain/bert-base-uncased')
        sentence_encoder = encoder.BERTEntityEncoder(max_length=80, pretrain_path=pretrain_path)
