...
Use the below steps to use the trained models
...
>>> import opennre
```
If you have more than one trained model, use the below code
```python
>>> model = opennre.get_model("custom_trained_model")
```

If you have only one trained model, use the below code
```python
>>> model = opennre.get_model("custom_model")
```

Syntax of the input
```python
>>> sample = {"token": ["In", "late", "1999", ",", "Esri", "released", "ArcMap", "8.0", ",", "which", "ran", "on", "the", "Microsoft", "Windows", "operating", "system", "."], "h": {"pos": [6, 7]}, "t": {"pos": [4, 5]}}
```
